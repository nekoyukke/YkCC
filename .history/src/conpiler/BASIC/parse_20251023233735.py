...
def parse(source:str, tokens:list[Token], addr:list[int] = [], reg:list[int] = [], Subreg:list[int] = []):
    ...
    Subroutineregs:list[list[int]] = [[]] # allocしたものだけですね
    fornumber:int = 0
    def ad():
        nonlocal pos
        pos += 1
    ...
    def allocsubreg():
        n = 0
        while True:
            if n not in useingSubroutinereg:
                useingSubroutinereg.append(n)
                return n
            n += 1
    def freesubreg(number:int):
        useingSubroutinereg.pop(useingSubroutinereg.index(number))
    def stmt():
        assembly = ""
        while cu().type != "EOF":
            strs = expr()
    ...
    def expr():
        ...
        match (cu().value):
            ...
            case "FOR":
                ...
                fornumber += 1
                res += "; to ramzero\n"
                res += f"SETII {mem}, r{computed[1][0]}\n"
                res += f"FORLOOP{number}:\n"
                res += stmt()
                # もしなら
                reg = allocreg()
                res += f"GETI r{reg}, {mem}"
                res += "; CMP for registr\n"
                res += f"CMP r{reg}, r{computed[1][0]}\n"
            ...
            case "PRINT":
                ...
                res += computed2[0]
                res += "; PRINT COMMAND\n"
                res += f"OUTPUT r{computed[1][0]}, r{computed2[1][0]}"
                return res
            case "INPUT":
                ad()
                res += "; OUTPUT\n"
                ...
                res += computed2[0]
                res += "; INPUT COMMAND\n"
                res += f"INPUT r{computed[1][0]}, r{computed2[1][0]}"
                return res
            case "END":
                # HALT
                return "; Halt\nHLT"
